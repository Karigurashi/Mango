...
from ...core.eExecutionStatus import EExecutionStatus
from ...core.workflowStreamEvent import EStreamEventType, WorkflowStreamEvent
...
class AgentNode(BaseNode):
    ...
    async def _CallAgentAsync(self, systemPrompt: str, userMessage: str) -> None:
        ...
        if self._agent is None or self._agentModelName != modelName:
            self._agent = AgentManager.CreateAgent(modelName, config=AgentConfig(maxTurns=25))
    ...
    def _OnAgentStreamEvent(self, event: AgentStreamEvent) -> None:
        ...
        if event.eventType == EAgentStreamEventType.TEXT_COMPLETE:
            ...
            self._PushEvent(EStreamEventType.AI_CONTENT, message=event.content)
        elif event.eventType == EAgentStreamEventType.ERROR:
            self._PushEvent(EStreamEventType.NODE_STATUS, status=EExecutionStatus.FAILED, message=event.content)

    def _PushEvent(self, eventType: EStreamEventType, *, message: str = "", status=None) -> None:
        eventBus = self.context.EventBus
        eventBus.Push(WorkflowStreamEvent(
            workflowId=self.context.WorkflowId,
            nodeId=self._streamNodeId,
            agentId=self._agent.agentId,
            eventType=eventType,
            status=status,
            message=message,
        ))
